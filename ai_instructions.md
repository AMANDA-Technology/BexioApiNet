...
- Caller-supplied static headers come from `IBexioConfiguration.DefaultRequestHeaders` (API client) and `BexioOAuthOptions.DefaultRequestHeaders` (token client — it is configured from the options, so the configuration cannot reach it), applied at each client-configuration site with `HttpClientExtension.ApplyDefaultRequestHeaders`. Do **not** drop its reserved-name skip (`Authorization`, `Accept`, `Host`) or its validation: the header collections append on a duplicate name instead of replacing, so an unguarded entry would send two values and break authentication, and a value carrying CR or LF is accepted by `TryAddWithoutValidation` and only rejected by the connection writer at send time. It must never throw — a diagnostic header may not take down an API call.
