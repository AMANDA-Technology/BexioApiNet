...
| **Connector Services** | Concrete services inheriting from `ConnectorService` (e.g., `ManualEntryService`, `BankAccountService`) or the shared `PositionService` (for the 7 `kb_position_*` types). They construct specific API paths (using static classes like `ManualEntryConfiguration`) and define typed methods for standard verbs and extended actions. | `src/BexioApiNet/Services/Connectors/` |
